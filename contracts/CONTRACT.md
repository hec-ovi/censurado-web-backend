...
- `AutomationStatusStore`: GetAutomationStatus, SetAutomationStatus (the settings' sibling
  singleton: the executor's heartbeat — clock, lane health, run in flight, queue — written each poll
  tick and read by the panel; a separate row so the two writers never clobber each other).
- `ScheduleStore`: UpsertSchedule, ScheduleBySlug, ListSchedules, DeleteSchedule, RecordScheduleRun
  ...
  `ScheduleRunsCap`) is written ONLY by `RecordScheduleRun`, which replaces an entry with the same
  run id in place; `UpsertSchedule` never touches it.
...
| GET | `/automation-settings` | `{settings:{...}}`: the single global automation-settings object ({} when never set). The shape inside is owned by the automation boxes (panel form + executor merge), not by this contract — EXCEPT the secret path `lanes.*.api_key`, which the backend redacts to `api_key_set: true` on every read unless an `admin:write` holder passes `?include_secrets=true` (the executor's lane; agent keys never receive the raw value). |
| GET | `/automation-status` | `{settings:{...}}`: the executor's last heartbeat ({} when it never reported). Same wrapper shape as the settings; the object inside (at, llama_ok, running, queued) is owned by the executor. |
| GET | `/schedules` | `{schedules:[{slug,name,cadence,times,weekdays,monthdays,mode,task,prompt,authors,enabled,runs,metadata,deleted,created_at,updated_at}]}`. `task` is `batch\|topics`: what fires, the article-edition batch or the topic-normalization sweep. `prompt` is the custom-run directive (empty = a generic whole-newsroom run). `cadence` is `daily\|weekly\|monthly`; `times` are canonical (deduped, sorted) `HH:MM` wall-clock fire times in the executor's local timezone; `weekdays` (0=Sunday..6) narrow a weekly cadence and `monthdays` (1..31) a monthly one; `mode` is `preview\|auto`; `runs` is the recent-run strip, newest first, capped at 20, each `{run_id,status,detail,started_at,finished_at}` with status `queued\|running\|ok\|failed`. `?include_deleted=true`. |
...
| POST | `/schedules` | `{name*, slug?, cadence?, times*, weekdays?, monthdays?, mode?, task?, prompt?, authors?, enabled?, metadata?}` (upsert, keyed on slug; `slug` defaults to the slugified name; schema [schedule.schema.json](schedule.schema.json)). `cadence` defaults to `daily`, `mode` to `preview`, and `task` to `batch` (`batch\|topics`); `times` must be valid `HH:MM` entries (deduped and sorted into canonical form); a weekly cadence needs at least one weekday (0=Sunday..6), a monthly one at least one monthday (1..31). The upsert never touches the runs strip. | 200 (400 on missing name / bad cadence, mode, task, time, or day) |
| DELETE | `/schedules/{slug}` | | 204 (404 on absent slug) |
| POST | `/schedules/{slug}/runs` | `{run_id*, status*, detail?, started_at?, finished_at?}` with status `queued\|running\|ok\|failed`: record one firing on the recent-run strip. A repeated `run_id` replaces its entry in place (the executor posts `queued` on enqueue, `running` at fire time, and the outcome at finish); the strip keeps the newest 20. | 200 the updated schedule (400 bad status/run_id, 404 absent slug) |
| PUT | `/automation-settings` | `{settings*: {...}}` (replaced wholesale, 64 KiB cap). Secret-aware on `lanes.*.api_key`: an omitted key keeps the stored one, a non-empty key replaces it, an explicit `""` clears it; the read-only `api_key_set` decoration never persists. | 200 echoes REDACTED (400 missing settings) |
| PUT | `/automation-status` | `{settings*: {...}}` (the executor's heartbeat, replaced wholesale each tick, 64 KiB cap) | 200 echoes (400 missing settings) |
