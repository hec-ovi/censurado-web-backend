...
func NewServerHandler(h *Handler, limiter *RateLimiter, mediaH *MediaHandler, readH *ReadHandler, opH *OperatorHandler) http.Handler {
	...
	if readH != nil {
		...
		mux.HandleFunc("GET /automation-settings", readH.ServeAutomationSettings)
		mux.HandleFunc("GET /automation-status", readH.ServeAutomationStatus)
	...
	if opH != nil {
		...
		mux.HandleFunc("PUT /recomendado", opH.ServeSetRecomendado)
		...
		mux.HandleFunc("PUT /automation-settings", opH.ServePutAutomationSettings)
		mux.HandleFunc("PUT /automation-status", opH.ServePutAutomationStatus)
